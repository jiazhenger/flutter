...
export default class Index extends React.Component{
	...
	onClick = () => {
		const { code, link, hasShow } = this.props
		
		if($fn.hasArray(code) || hasShow ){
	...
	render(){
		const { title, type, children, pro, keyword, code, isGray, link, url, order, noOrder, isValue, hasShow } = this.props
		const { show } = this.state
		return (
			<section>
				<TitleComponent show={show} hasArrow={$fn.hasArray(code) || hasShow} hasClick={$fn.isString(link)} order={noOrder ? null : order} onClick={this.onClick}>
					{
				...
				{
						show !== undefined && ($fn.hasArray(code) || hasShow) ? (
